...
public class HLGameA {
	...
	public HLGameA(double money, int decks, int min, int max, double multiplier, int a)
	{
	...
	public boolean playGame()
	{
		...
		{
			System.out.println("Decks remaining: " + df.format((double)((decks*52)-d.index)/52));
		...
		resolveHand();
			...
			{
				System.out.println("shuffled with "+ (decks*52-d.getIndex())+" remaining in shoe");
			d.shuffle();
			count = icount;
			}
	...
	{
		double cardsLeft = (decks*52)-d.getIndex();
		double decksLeft= cardsLeft/52.0;
		int trueCount=(int)( count/decksLeft);
		if (trueCount+aggression<1)
		{
			bet=min;
			return min;
		}
		int x = (int) (min* Math.pow((double)(trueCount+aggression), multi));
		if (x>max)
		{
			bet=max;
			return max;
